<header class="shadow-lg pb-2 bg-white">
  ...
  <nav id="nav-menu"
    ...
    <a href="{{ url('/kategori') }}" class="flex-grow border-e border-[#1F2859] text-center font-medium py-2 
        {{ Request::is('kategori*') ? 'bg-[#1F2859] text-white rounded-full' : 'hover:text-[#1F2859]' }}">
      Kategori
    </a>

    <a href="{{ url('/predikat') }}" class="flex-grow border-e border-[#1F2859] text-center font-medium py-2 
        {{ Request::is('predikat*') ? 'bg-[#1F2859] text-white rounded-full' : 'hover:text-[#1F2859]' }}">
      Predikat
    </a>

    <a href="{{ url('/wilayah') }}" class="flex-grow border-e border-[#1F2859] text-center font-medium py-2 
        {{ Request::is('wilayah*') ? 'bg-[#1F2859] text-white rounded-full' : 'hover:text-[#1F2859]' }}">
      Wilayah
    </a>

    <a href="{{ url('/tahun') }}" class="flex-grow border-e border-[#1F2859] text-center font-medium py-2 
        {{ Request::is('tahun*') ? 'bg-[#1F2859] text-white rounded-full' : 'hover:text-[#1F2859]' }}">
      Tahun
    </a>
    <a href="{{ url('/data-mining') }}" class="flex-grow text-center font-medium py-2 
        {{ Request::is('data-mining*') ? 'bg-[#1F2859] text-white rounded-full' : 'hover:text-[#1F2859]' }}">
      Hasil Data Mining
    </a>
  </nav>
